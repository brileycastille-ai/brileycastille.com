...
create table if not exists essays (id bigint generated always as identity primary key, slug text unique not null, title text not null, dek text, topics text[] not null default '{}', content_html text not null, status text default 'published', published_at timestamptz default now(), created_by uuid references auth.users);
