...
type Question = { id: number; question: string; context: string; votes: number; created_at: string };
...
export default function DiscussionPage() {
  ...
  async function reply(event: FormEvent, questionId: number) {
    ...
    const response = await fetch("/api/comments", {
      ...
      body: JSON.stringify({ essay_slug: "question-box", paragraph_id: `question-${questionId}`, body, display_name: names[questionId] }),
    });
    if (response.ok) {
    ...
    <section className="discussion-list">
      ...
      {questions.map((question) => {
        ...
        return <article className="discussion-thread" id={`question-${question.id}`} key={question.id}>
          <div className="question-heading"><span>{question.votes} votes</span><div><small>{question.context === "standalone" ? "Reader question" : question.context}</small><h2>{question.question}</h2></div></div>
          ...
          <form onSubmit={(event) => reply(event, question.id)}><label>Join this discussion</label><input value={names[question.id] || ""} onChange={(event) => setNames((current) => ({ ...current, [question.id]: event.target.value }))} placeholder="Name (optional)"/><textarea required rows={3} value={replies[question.id] || ""} onChange={(event) => setReplies((current) => ({ ...current, [question.id]: event.target.value }))} placeholder="Write a public reply"/><button type="submit">Post reply</button></form>
        </article>;
      })}
    </section>
  </main>;
