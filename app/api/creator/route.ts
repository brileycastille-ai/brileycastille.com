...
export async function GET(request: Request) {
  try {
    ...
    const [{data: questions, error: questionError}, {data: comments, error: commentError}, {data: ideas, error: ideaError}, {data: reporting, error: reportingError}, {data: essays, error: essayError}, {data: settings, error: settingsError}, usersResult] = await Promise.all([
      ...
      creator.supabase.from("essays").select("slug,title,dek,topics,published_at").eq("status", "published").order("published_at", {ascending: false}),
    ...
    if (questionError || commentError || ideaError || reportingError || essayError || settingsError || usersResult.error) throw questionError || commentError || ideaError || reportingError || essayError || settingsError || usersResult.error;
    ...
    return Response.json({questions: questions || [], comments: comments || [], ideas: ideas || [], reporting: reporting || [], essays: essays || [], settings: settings || null, users});
  } catch {
